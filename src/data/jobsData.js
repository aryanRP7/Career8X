// ─────────────────────────────────────────────
//  JOBS DATA — edit this file to add/remove jobs
//  Optional: payRange, myNotes, resumeFile, coverLetterFile
//
//  Both files go in: public/myfiles/resumes/
// ─────────────────────────────────────────────

export const jobsData = [
  {
    id: 1,
    companyName: "BuiltIn (Job Platform)",
    jobTitle: "Entry-Level UI/UX Jobs (Multiple Companies)",
    location: "USA",
    workMode: "Varies (Remote/Hybrid/Onsite)",
    workExperience: "0–2 years",
    applyLink: "https://builtin.com/jobs/design-ux/entry-level",
    payRange: "Varies",
    resumeFile: "resume_konrad_2026.pdf",
    coverLetterFile: "coverletter_konrad_2026.pdf",
    jobDescription: [
      "Aggregates entry-level UI/UX roles from multiple companies",
      "Includes UI Designer, UX Designer, and Product Designer roles",
      "Focus on beginner-friendly design opportunities",
      "Jobs span startups, agencies, and tech companies",
      "Direct application through listed job postings",
    ],
    myNotes: "This position is open",
  },
  {
    id: 2,
    companyName: "Simplify Jobs (Job Platform)",
    jobTitle: "Entry-Level UI/UX Jobs (Curated Listings)",
    location: "USA",
    workMode: "Varies",
    workExperience: "0–2 years",
    applyLink: "https://simplify.jobs/l/Entry-Level-UI-UX-Jobs",
    payRange: "Varies",
    resumeFile: "resume_konrad_2026.pdf",
    coverLetterFile: "coverletter_konrad_2026.pdf",
    jobDescription: [
      "Curated list of entry-level UI/UX design roles",
      "Includes internships and junior design roles",
      "Focus on beginner-friendly tech companies",
      "UI, UX, and product design opportunities included",
      "External applications via company career pages",
    ],
    myNotes: "This position is open",
  },
  {
    id: 3,
    companyName: "Konrad Group",
    jobTitle: "Associate UI/UX Designer",
    location: "New York, USA",
    workMode: "Hybrid",
    workExperience: "1–3 years",
    applyLink: "https://www.konrad.com/careers/job/ui-ux-designer_7722161003",
    payRange: "$80,000 – $100,000 / yr",
    resumeFile: "resume_konrad_2026.pdf",
    coverLetterFile: "coverletter_konrad_2026.pdf",
    jobDescription: [
      "Conduct user research and usability testing",
      "Design wireframes, prototypes, and high-fidelity UI",
      "Create mobile, web, and tablet digital experiences",
      "Collaborate with developers and business teams",
      "Identify design opportunities and solve UX problems",
    ],
    myNotes: "This position is open",
  },
  {
    id: 4,
    companyName: "WebFX (Nutshell)",
    jobTitle: "Jr. UI Designer",
    location: "Ann Arbor, MI, USA",
    workMode: "On-site",
    workExperience: "0–2 years",
    applyLink:
      "https://jobs.lever.co/webfx/d2c58878-c49f-4802-98aa-807e06289aa6",
    payRange: "Competitive",
    resumeFile: "resume_konrad_2026.pdf",
    coverLetterFile: "coverletter_konrad_2026.pdf",
    jobDescription: [
      "Create UI designs with strong visual hierarchy and typography",
      "Use Figma and Adobe Creative Suite for design work",
      "Apply responsive web design principles",
      "Work with developers on implementation",
      "Support UX workflows like wireframing and prototyping",
    ],
    myNotes: "This position is open",
  },
  {
    id: 5,
    companyName: "WebFX (Nutshell)",
    jobTitle: "Jr. UX Designer",
    location: "Ann Arbor, MI, USA",
    workMode: "On-site",
    workExperience: "0–2 years",
    applyLink:
      "https://jobs.lever.co/webfx/05aa0ddb-f4c8-46f1-95fb-f9d5c0ee40a7",
    payRange: "Competitive",
    resumeFile: "resume_konrad_2026.pdf",
    coverLetterFile: "coverletter_konrad_2026.pdf",
    jobDescription: [
      "Lead UX workflows including user journeys and task analysis",
      "Create wireframes, prototypes, and information architecture",
      "Conduct user research and usability testing",
      "Collaborate with UX writers and researchers",
      "Contribute to design systems and product strategy",
    ],
    myNotes: "This position is open",
  },
  {
    id: 6,
    companyName: "MITRE",
    jobTitle: "Early Career UX/UI Engineer",
    location: "Bedford, MA, USA",
    workMode: "Hybrid",
    workExperience: "<1 year",
    applyLink:
      "https://jobright.ai/jobs/info/69fbeed5c39b9a66c0c551b4?experienceLevel=entry_level&visit=arts-design-ux-research",
    payRange: "$79,000 – $119,000 / yr",
    resumeFile: "resume_konrad_2026.pdf",
    coverLetterFile: "coverletter_konrad_2026.pdf",
    jobDescription: [
      "Design UI/UX for government-facing software systems",
      "Build wireframes, mockups, and interactive prototypes",
      "Work with engineers on front-end development",
      "Apply user-centered design and usability testing",
      "Contribute to design systems and UI guidelines",
    ],
    myNotes: "This position is open",
  },
  {
    id: 7,
    companyName: "Symbotic",
    jobTitle: "User Experience Designer",
    location: "Wilmington, MA, USA",
    workMode: "Hybrid/On-site",
    workExperience: "2+ years",
    applyLink: "https://www.symbotic.com/careers/open-positions/R5747/",
    payRange: "$96,000 – $132,000 / yr",
    resumeFile: "resume_konrad_2026.pdf",
    coverLetterFile: "coverletter_konrad_2026.pdf",
    jobDescription: [
      "Design UX for robotics and supply chain systems",
      "Create wireframes, concepts, and high-fidelity UI",
      "Develop interactive prototypes for testing",
      "Collaborate with engineers and product managers",
      "Maintain and improve design systems",
    ],
    myNotes: "This position is open",
  },
  {
    id: 8,
    companyName: "Sangoma",
    jobTitle: "UX/UI Designer (Desk Phones)",
    location: "USA (Remote - EST/CST preferred)",
    workMode: "Remote",
    workExperience: "1–3 years",
    applyLink: "https://apply.workable.com/sangoma-3/j/3D1BCABC18/",
    payRange: "$85,000 – $95,000 / yr",
    resumeFile: "resume_konrad_2026.pdf",
    coverLetterFile: "coverletter_konrad_2026.pdf",
    jobDescription: [
      "Design UX for embedded desk phone systems",
      "Create workflow-driven interfaces for telephony functions",
      "Design wireframes, flows, and interaction models",
      "Work with firmware engineers on implementation",
      "Handle usability testing and edge-case scenarios",
    ],
    coverLetterText: `COVER LETTER
Date: 05/07/2026

Dear Hiring Team,

I would be excited to contribute elevated visual design, thoughtful execution, and a collaborative creative mindset to the team.

Warm regards,
Falguni Dekate`,
    myNotes: "This position is open",
  },
  {
  id: 9,
  companyName: "Accenture Federal Services",
  jobTitle: "Cleared Junior UI/UX Designer",
  location: "Washington, DC, USA",
  workMode: "On-site",
  workExperience: "2–3+ years",
  applyLink: "https://job-boards.greenhouse.io/accenturefederalservices/jobs/4677398006?gh_jid=4677398006&gh_src=d8gyyfgu6us",
  payRange: "$53,900 – $120,100 / yr",
  resumeFile: "resume_konrad_2026.pdf",
  coverLetterFile: "coverletter_konrad_2026.pdf",
  jobDescription: [
    "Help define and maintain design systems and reusable templates",
    "Create wireframes and workflows for consolidated systems",
    "Design user flows, prototypes, and interface solutions",
    "Conduct user interviews and present designs to stakeholders",
    "Collaborate with developers and cross-functional teams",
    "Use government-specified tools to build interactive content",
    "Contribute to UX strategy and usability improvements",
    "Work on federal systems requiring security clearance",
  ],
  coverLetterText: `COVER LETTER
Date: 05/07/2026

Dear Hiring Team,

I would be excited to contribute elevated visual design, thoughtful execution, and a collaborative creative mindset to the team.

Warm regards,
John Doe`,
  myNotes: "This position is open",
},
];
